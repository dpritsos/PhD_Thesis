...
The table \ref{chap:eval_methods:tbl:bin_confusion} has $280$. In the case of open-set classification some of the samples from the known classes are remaining as non-classification, i.e as \textit{unknown}. Their total number is $24$ (the same of the last row without the 180 noise samples). The total number of the $200$ noise/unknown samples are either left (correctly) unclassified (as unknown) or are mixed as one of the known classes. In this case $20$ of them, where is the some of the last column without the $180$ noise pages.
